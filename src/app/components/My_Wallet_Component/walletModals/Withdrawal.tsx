...
const Withdrawal: React.FC = () => {
  const { setIsWithdrawalOpen, setIsManualInputOpen, setIsHurrayOpen } =
    useMyContext();
  ...
  const handleManualInput = () => {
    localStorage.setItem("withdrawalAmount", amount.toLocaleString());
    setIsManualInputOpen(true);
    setIsWithdrawalOpen(false);
  };

  const token =
    "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VySWQiOiI2N2Q0MWQ0YTFjMWRjZTk1YWE3YWNmYTMiLCJpYXQiOjE3NDE5NTQ1MDIsImV4cCI6MTc0MjA0MDkwMn0.LmiwSvMzhArV5huuxv0jkYvc59ewb0c_UTcnY52XKAE";
        ...
        const fetchBalance = async () => {
          try {
            const response = await axios.get(
              "https://altbucks-server-t.onrender.com/api/v1/wallet-balance",
              {
                headers: {
  ...
  const handleTopUp = async () => {
    // Retrieve the existing paymentMethod object from localStorage
    const paymentMethod = JSON.parse(
      localStorage.getItem("paymentMethod") || "{}"
    );

    // Add the withdrawalAmount to the paymentMethod object
    paymentMethod.amount = amount.toLocaleString();

    // Save the updated paymentMethod object back to localStorage
    ...
    const data = {
      ...
      paymentGateway: paymentMethod.paymentGateway || "stripe"
    };
    ...
    try {
      ...
      const response = await axios.post(
        "https://altbucks-server-t.onrender.com/api/v1/pay",
        data,
        {
          headers: {
            Authorization: `Bearer ${token}`,
    ...
    <div className="fixed inset-0 bg-[#00000098] flex items-center justify-center px-4 z-20">
      <div className="w-full max-w-3xl bg-white rounded-lg shadow-md p-6 pb-11 relative">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-2">
          ...
          <div className="bg-gradient-to-br from-[#E9E9E9] to-[#fff] px-6 py-8 rounded-lg shadow-500">
            <div className="flex items-center space-x-2">
              ...
              <span className="text-[#898989] font-medium text-sm">
        ...
        <p className="text-[#667085] text-xs font-[Inter] mt-12">
          Withdraw directly into the selected account. <br /> Transaction might
          take some minutes to propagate. Please relax ;)
        </p>
