...
interface TaskSummaryCardProps {
  overView: OverviewData | null; // Expect an object with these fields
}

const TaskSummaryCard: React.FC<TaskSummaryCardProps> = ({ overView }) => {
  // Handle the case where overView might be null
  if (!overView) return null;

  const { cancelledTasks, completedTasks, pendingTasks } = overView;
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm p-6">
      <div className="flex justify-between items-center mb-6">
        ...
        <span className="text-2xl font-bold text-[#2877EA]">{totalTasks}</span>
      </div>
